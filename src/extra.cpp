...
std::chrono::time_point<std::chrono::steady_clock> glutGUI::lastMoveTime = std::chrono::steady_clock::now();
const int glutGUI::IDLE_THRESHOLD_MS = 100; // tempo de inatividade do mouse para considerar que ele parou
...
void glutGUI::timer(int time)
{
    ...
    glutTimerFunc(16, glutGUI::timer, 0);  // Chama a função novamente após 16 milissegundos (~60 FPS)
